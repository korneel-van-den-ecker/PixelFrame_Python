...
class FunctieGenerator:
    ...
    def TekenFunctie(self, huidigBeeld, functie):
        ...
        for x in range(self.frame.breedte):
            ...
            radiaalPositie = ((self.periode / self.frame.breedte) * x) + \
                (self.periode / self.aantalBeelden * huidigBeeld)
            # sinus waarde van -1.0 tem 1.0
            # DE BEWERKING altijd herschalen van -1 tot 1
            if(bewerking == "cos"):
                amplitude = math.cos(radiaalPositie)
            if(bewerking == "sin"):
                amplitude = a * math.sin(radiaalPositie)
            if(bewerking == "funcky"):
                amplitude = a * math.sin(radiaalPositie)
            ...
            if(bewerking == "pulse"):
                ...
                if(radiaalPositie == 0):
                    ...
                    amplitude = math.sin(
                        math.pi * radiaalPositie)/(math.pi*radiaalPositie)
            if(bewerking == "tan"):
            ...
            if(bewerking == "cos" or bewerking == "sin" or bewerking == "tan" or bewerking == "pulse"):
                frameAmplitude = math.ceil(
                    (self.frame.hoogte/self.zoom) - amplitude*self.frame.hoogte/self.zoom)
            if(bewerking == "pow" or bewerking == "lin" or bewerking == "sqrt"):
                frameAmplitude = math.ceil(
                    self.frame.hoogte - amplitude*self.frame.hoogte)
            ...
            if(self.flikkeringEffect):
                ...
                if(self.brightnessEffect):
                    ...
                    if(bright == 1):
                        self.brightRichting = True
                    self.frame.zetKleur(x, frameAmplitude,
                                        self.kleurLijn, bright)
                else:
                    # normaal
                    self.frame.zetKleur(x, frameAmplitude, self.kleurLijn, 1)
